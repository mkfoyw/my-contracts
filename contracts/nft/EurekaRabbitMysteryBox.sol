...
contract EurekaRabbitMysteryBox is ERC1155, Ownable, ERC1155Burnable {
    ...
    uint16 public constant TOTAL_COUNT = 3261; // The max tokens count
    ...
    mapping(address => uint256) public numberMintedPublic;
    mapping(address => uint256) public numberMintedWhite;
    mapping(address => uint256) public numberMintedFree;
    ...
    function setStatusAndPrice(Status newMintStatus, uint256 newMintPrice)
        external
        onlyOwner
    {
    ...
    function setFreeMintTime(uint256 newStartTime, uint256 newEndTime)
        external
        onlyOwner
    {
        ...
        returns (uint256[] memory tokenIds, uint256[] memory amounts)
    {
        ...
        if (count < amountMinted) {
            uint256[] memory realTokenIds = new uint256[](count);
    ...
    modifier pubMintCheck(uint256 amount) {
        ...
        require(
            amountMinted + amount <=
                TOTAL_COUNT - availableCountReserved - availableCountFreeMint,
            "102"
        );
        _;
    }
    ...
    function whiteMint(
        ...
        bytes memory token
    ) external payable pubMintCheck(amount) {
        require(status == Status.White, "103");
        require(_recover(_hash(salt, msg.sender), token) == whiteSigner, "400");
        require(
            numberMintedWhite[msg.sender] + amount <= white_max_mint_count,
            "106"
        );
        tokenId++;
        amountMinted += amount;
    ...
    function mint(uint256 amount) external payable pubMintCheck(amount) {
        ...
        require(
            numberMintedPublic[msg.sender] + amount <= public_max_mint_count,
            "107"
        );
        tokenId++;
        amountMinted += amount;
    ...
    function freeMint(
        ...
        require(
            TimeUtil.currentTime() >= freeMintStartTime &&
                TimeUtil.currentTime() <= freeMintEndTime,
            "105"
        );
        require(_recover(_hash(salt, msg.sender), token) == freeSigner, "400");
        require(
            numberMintedFree[msg.sender] + amount <= free_max_mint_count,
            "108"
        );
    ...
    function airdropGiveaway(
        ...
        uint256 tempAmountMinted = amountMinted;
    ...
    // Allowing direct drop for giveaway
    function airdropGiveawayReserve(
        address[] calldata to,
        uint256[] calldata amountToMint
    ) external onlyOwner {
        require(to.length == amountToMint.length, "400");
        uint256 tempTokenId = tokenId;
        uint256 tempAmountMinted = amountMinted;
        uint256 tempAvailableCountReserved = availableCountReserved;
        for (uint256 i = 0; i < to.length; i++) {
            tempTokenId++;
    ...
    function airdropGiveawayFree(
        ...
        uint256 tempTokenId = tokenId;
        uint256 tempAmountMinted = amountMinted;
        uint256 tempAvailableCountFree = availableCountFreeMint;
        for (uint256 i = 0; i < to.length; i++) {
            tempTokenId++;
